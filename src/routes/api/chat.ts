    ...
    handlers: {
      POST: async ({ request }) => {
        ...
        try {
          ...
          const { JARVIS_SYSTEM_PROMPT, buildCompanyContext } =
            await import("@/lib/jarvis-prompt");
          ...
          return Response.json({
            text,
            employeeCode: employee ? code : "JARVIS",
          });
        } catch (error) {
          const message =
          ...
          if (employeeCode) {
            ...
            try {
              const { onEmployeeChatError } =
                await import("@/lib/employees.server");
              await onEmployeeChatError(
                employeeCode as "A" | "B" | "C" | "D" | "E" | "F",
              );
            } catch (syncError) {
              console.error(
                "employee status sync (chat error) failed:",
                syncError,
              );
            }
          }
...
async function callGemini(
  ...
  if (!res.ok) {
    ...
    if (res.status === 429)
      throw new Error(
        "Gemini APIのレート上限に達しました。少し待って再送してください。",
      );
    throw new Error(
...
async function callLovableGateway(
  ...
  if (!res.ok) {
    ...
    throw new Error(
      `AIに接続できませんでした（${res.status}）: ${detail.slice(0, 200)}`,
    );
  }
  ...
  return (
    data.choices?.[0]?.message?.content?.trim() || "応答の取得に失敗しました。"
  );
}
